...
export default function TargetWeightInput({ onNext, onUpdate, data }: TargetWeightInputProps) {
  const [weight, setWeight] = useState(data.targetWeight?.toString() || "");
  ...
  const handleWeightChange = (value: string) => {
    ...
    if (!isNaN(numValue) && numValue > 0) {
      onUpdate({ targetWeight: numValue, weightUnit: unit });
    }
  };
  ...
  const handleUnitChange = (newUnit: string) => {
    ...
    if (!isNaN(numValue) && numValue > 0) {
      onUpdate({ targetWeight: numValue, weightUnit: newUnit });
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16">
        <div className="text-center mb-8">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-3">
            What is your target weight ({unit})?
          </h1>
        </div>
